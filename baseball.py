...
result = spark.sql("select batter, sum(Hit) as TH,sum(atBat) as TB, round(SUM(Hit)/SUM(atBat),3) as AVG \
...
avg_df = spark.sql(
    """Select * , split(concat(case when batter is NULL then "" else batter END, " ", \
    case when AVG is NULL then "" else AVG END), " ") AS avg_array FROM avg"""
)
avg_df.show()

# Count Vectorize
count_vectorizer = CountVectorizer(inputCol="avg_array", outputCol="array_vector")
